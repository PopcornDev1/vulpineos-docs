...
| `GET` | `/v1/system/metrics` | Yes | Request, error, backlog, and worker-health metrics |
...
Named sessions are tenant-scoped. Stale saved sessions expire automatically based on the server-side `AUTH_SESSION_TTL` setting, and authenticated extraction metadata includes whether a stale session was discarded before re-login.
...
Returns recent extraction tasks and metadata for the authenticated tenant. Supports `status`, `monitor_id`, `failure_family`, `retry_class`, and `since_hours` query parameters.
