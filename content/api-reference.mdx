...
| `GET` | `/v1/system/metrics` | Yes | Request, error, backlog, worker-health, and artifact-retention metrics |
...
History rows also include `mode` so operators can distinguish standard extract, authenticated, and visual runs. Authenticated rows additionally expose `login_trace_id` when a persisted redacted login trace exists.
