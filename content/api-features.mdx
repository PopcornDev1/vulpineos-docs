...
| Operational alerts | `GET /v1/system/metrics` | Available in the API service |
...
Extraction accepts a JSON Schema or natural-language prompt, supports sync and async execution, can opt into pagination, and returns usage metadata such as completeness, token count, retries, and estimated cost. Failed async tasks and extraction-history rows also carry normalized failure classification for retry automation and operator diagnosis, plus queue-level attempt metadata when transient retries are used. Extraction history can be filtered by status, monitor, failure family, retry class, and recent time window. Paid/private surfaces add authenticated extraction, persisted redacted login traces for authenticated failures and relogins, visual extraction, browser audio capture and transcription, mobile device session controls, and hosted mobile worker registry visibility.
