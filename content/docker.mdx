...
```bash
./vulpineos --serve --binary ./browser/camoufox --port 8443 --api-key "$VULPINE_API_KEY"
```
...
services:
  vulpineos:
    ...
    ports:
      - "8443:8443"
    environment:
      - VULPINE_API_KEY=${VULPINE_API_KEY:-changeme}
    volumes:
...
2. Launches `vulpineos --serve --binary ./browser/camoufox --port 8443`
3. Adds `--api-key "$VULPINE_API_KEY"` when that environment variable is set
4. Adds `--tls-cert` and `--tls-key` when both TLS environment variables are set
...
- The default access key in `docker-compose.yml` is only a placeholder; replace it before
