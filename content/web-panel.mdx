...
JSON script editor for the agent scripting DSL. This page is still partial: only
the navigate path is wired to the backend today, and the remaining actions are
...
the visible address bar after load.
...
35 control message types across 10 domains, verified against `internal/remote/api.go`:
